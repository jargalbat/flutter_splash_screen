    ...
    animation = tween.animate(curvedAnim)
      ..addListener(() {
        if (animationController.status == AnimationStatus.completed) {
          if (widget.isReverse)
            Future.delayed(Duration(milliseconds: widget.delayReverse))
...
class FadeIn extends StatelessWidget {
  ...
  FadeIn({this.delay, this.child, this.begin, this.end});
  ...
  Widget build(BuildContext context) {
    final tween = MultiTrackTween([
      ...
      Track("translateX").add(Duration(milliseconds: 1000),
          Tween(begin: begin ?? 0.0, end: end ?? 40.0),
